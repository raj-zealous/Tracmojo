...
- (void)setYLabels:(NSArray *)yLabels
{
    ...
    int verticalvaluey;
    verticalvaluey =0;
   // UIView *viewbg =[[UIView alloc]initWithFrame:CGRectMake(40, 73, 265, 183)];
//    UIImageView *viewbg=[[UIImageView alloc]initWithFrame:CGRectMake(40, 73, 265, 183)];
//    [viewbg setImage:[UIImage imageNamed:@"earningview.png"]];
//    [self addSubview:viewbg];

    
    while (num > 0) {

        PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(0.0, (_chartCavanHeight+10 -  index * yStepHeight), _chartMargin+5, _yLabelHeight)];
        [label setTextAlignment:NSTextAlignmentRight];
        label.text = [NSString stringWithFormat:@"%1.f", _yValueMin + (yStep * index)];
             [self addSubview:label];
        UIImageView *xaxislineimg=[[UIImageView alloc]initWithFrame:CGRectMake(40,(_chartCavanHeight+10 -  index * yStepHeight)+5,265,0.5)];
        xaxislineimg.alpha =0.6;
        [xaxislineimg setImage:[UIImage imageNamed:@"vertical_line.png"]];
        [self addSubview:xaxislineimg];
        verticalvaluey=verticalvaluey+22.5;
        index += 1;
        num -= 1;
    }
}
    ...
    if (_showLabel) {
        _xLabelWidth = _chartCavanWidth / [xLabels count];
            ...
            for (int index = 0; index < xLabels.count; index++) {
                ...
                //   PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(2 * _chartMargin +  (index * _xLabelWidth) - (_xLabelWidth / 2)-10, _chartMargin + _chartCavanHeight, _xLabelWidth, _chartMargin+10)];
                //NSLog(@"%f",2 * _chartMargin +  (index * _xLabelWidth) - (_xLabelWidth / 2));
                PNChartLabel *label = [[PNChartLabel alloc] initWithFrame:CGRectMake(2 * _chartMargin +  (index * _xLabelWidth) - (_xLabelWidth / 2)-5, _chartMargin + _chartCavanHeight, _xLabelWidth, _chartMargin+10)];
                ...
                if (NSFoundationVersionNumber > NSFoundationVersionNumber_iOS_6_1) {
                    ...
                    // do stuff for iOS 7 and newer
                    UIImageView *xaxislineimg=[[UIImageView alloc]initWithFrame:CGRectMake(60+verticalvalue,-13,0.5, 200)];
                    xaxislineimg.alpha =0.4;
                    [xaxislineimg setImage:[UIImage imageNamed:@"horizontal_line.png"]];
                    [self addSubview:xaxislineimg];
                    verticalvalue=verticalvalue+22.5;
                }
                else {
                    
                    // do stuff for older versions than iOS 7
               
                UIImageView *xaxislineimg=[[UIImageView alloc]initWithFrame:CGRectMake(60+verticalvalue,-13,1, 200)];
                xaxislineimg.alpha =0.4;
                [xaxislineimg setImage:[UIImage imageNamed:@"horizontal_line.png"]];
                [self addSubview:xaxislineimg];
                verticalvalue=verticalvalue+22.5;
...
- (void)strokeChart
{
    ...
    for (NSUInteger lineIndex = 0; lineIndex < self.chartData.count; lineIndex++) {
        ...
        for (NSUInteger i = 0; i < chartData.itemCount; i++) {
            ...
            point = CGPointMake(2 * _chartMargin +  (i * _xLabelWidth)-7, _chartCavanHeight - (innerGrade * _chartCavanHeight) + (_yLabelHeight / 2)+10);
...
- (void)setChartData:(NSArray *)data
{
    if (data != _chartData) {
        ...
        for (PNLineChartData *chartData in data) {
            ...
            //NSLog(@"chart =%@",self.chartLineArray);
            //NSLog(@"chartdata =%@",chartData);
            
            for (NSUInteger i = 0; i < chartData.itemCount; i++) {
    
                //NSLog(@"yvalue =%f",chartData.getData(i).y);
                yValue = chartData.getData(i).y;
