...
- (void)drawProgressBackground:(CGContextRef)context inRect:(CGRect)rect {
    ...
    [roundedRect fill];
     ...
     self.rect_background=rect.size.width;
...
- (void)drawStripes:(CGContextRef)context inRect:(CGRect)rect {
    ...
    while (xStart < rect.size.width) {
        ...
        CGContextMoveToPoint(context, xStart+10, height - y);
        CGContextAddLineToPoint(context, xStart+10 - width * 0.25, 0);
        CGContextAddLineToPoint(context, xStart+10 - width * 0.75, 0);
        CGContextAddLineToPoint(context, xStart+10 - width * 0.50, height - y);
        CGContextClosePath(context);
        CGContextFillPath(context);
        CGContextRestoreGState(context);
        xStart += width;
...
#pragma Feedback 16/3
    if (rect.size.width > 10) {
      ...
      //  [label drawTextInRect:CGRectMake(rect.origin.x + 6, rect.origin.y, rect.size.width-12, rect.size.height)];
       // [label drawTextInRect:CGRectMake(rect.origin.x + 3, rect.origin.y, rect.size.width-5, rect.size.height)];
        
         [label drawTextInRect:CGRectMake(rect.origin.x + 3, rect.origin.y, self.rect_background/1.6, rect.size.height)];
