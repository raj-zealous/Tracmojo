...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if ([correspondingIndexPath subRow] == 0)
    {
        ...
        if (expandableCell.isExpandable)
        {
            ...
            if (isExpanded)
            {
                expandableCell.accessoryView.transform = CGAffineTransformMakeRotation(M_PI_2);
            }
            else
            {
...
-(void)scrollViewDidScroll:(UIScrollView *)scrollView
{
    [_SKSTableViewDelegate scrollViewDidScroll:scrollView];
}

-(void)scrollViewDidEndDecelerating:(UIScrollView *)scrollView
{
    [_SKSTableViewDelegate scrollViewDidEndDecelerating:scrollView];
        ...
        if (cell.isExpandable)
        {
            ...
            if (_indexPath)
            {
                ...
                if (cell.isExpanded)
                {
                    ...
                    [self insertRowsAtIndexPaths:expandedIndexPaths withRowAnimation:UITableViewRowAnimationBottom];
                }
                else
                {
                    [self setExpanded:NO forCellAtIndexPath:correspondingIndexPath];
                    [self deleteRowsAtIndexPaths:expandedIndexPaths withRowAnimation:UITableViewRowAnimationBottom];
                }
